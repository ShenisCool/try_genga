...
__host__ int Host::assignInformat(char *ff, int &format, int E){
	...
	for(int i = 0; i < def_Ninformat; ++i){
		...
		if(strcmp(ff, fileFormat[i]) == 0){//判断输入格式和可行格式一致
			format = i;
			check = 1;
...
__host__ void Host::Halloc(){
	...
	//exp = (double*)malloc(Nst*sizeof(double));
	...
	P.Useheatedregion = def_Useheatedregion;
	P.Usetimevofaccrate = def_Usetimevofaccrate;
	P.Usegasaccre = def_Usegasaccre;
	P.Usetimevofsl = def_Usetimevofsl;
	...
	P.G_turstralpha = def_G_turstralpha;        //Gas turbulent strength alphad
	P.G_accrate_0 = def_G_accrate_0;        //Gas accretion rate_0
	P.G_L_s0 = def_G_L_s0;            //Gas stellar luminosity_0 
	P.G_diskvis_alpha = def_G_diskvis_alpha;        //Gas disk_viscous alpha
	...
	char iformat[def_Ninformat * 5];//输入输出格式，五个五个字连成一条字符串
	...
	for(int st = 0; st < Nst; ++st){
		...
		int pos = 0;		
		for(int f = -1; f < def_Ninformat; ++f){//Ninformat是定义的输入数量
			char ff[5];
			int n = 0;
			...
			er = assignInformat(ff, GSF[st].informat[f], 1);//这句太重要了
			if(er == 2) break;
...
__host__ int Host::readparam(FILE *paramfile, int st, int argc, char*argv[]){
	...
	for(int j = 0; j < 1000; ++j){ //loop around all lines in the param.dat file
		...
		if(strcmp(sp, "Use heated region =") == 0){
			if(st == 0){
				er = fscanf (paramfile, "%d", &P.Useheatedregion);
				if(er <= 0){
					printf("Error: Use heated region value is not valid!\n");
					return 0;
				}
			}
			else{
		...
		if(strcmp(sp, "Use time evolution of accretion rate =") == 0){
			if(st == 0){
				er = fscanf (paramfile, "%d", &P.Usetimevofaccrate);
				if(er <= 0){
					printf("Error: Use time evolution of accretion rate value is not valid!\n");
					return 0;
				}
			}
			else{
		...
		if(strcmp(sp, "Use gas accretion =") == 0){
			if(st == 0){
				er = fscanf (paramfile, "%d", &P.Usegasaccre);
				if(er <= 0){
					printf("Error: Use gas accretion value is not valid!\n");
					return 0;
				}
			}
			else{
		...
		if(strcmp(sp, "Use time evolution of stellar luminosity =") == 0){
			if(st == 0){
				er = fscanf (paramfile, "%d", &P.Usetimevofsl);
				if(er <= 0){
					printf("Error: Use time evolution of stellar luminosity value is not valid!\n");
					return 0;
				}
			}
			else{
		...
		if(strcmp(sp, "Gas turbulent strength alphad =") == 0){
			if(st == 0){
				er = fscanf (paramfile, "%lf", &P.G_turstralpha);
				if(er <= 0 || P.G_turstralpha < 0){
					printf("Error: Gas turbulent strength alphad value is not valid!\n");
					return 0;
				}
		...
		if(strcmp(sp, "Gas accretion rate_0 =") == 0){
			if(st == 0){
				er = fscanf (paramfile, "%lf", &P.G_accrate_0);
				if(er <= 0 || P.G_accrate_0 < 0){
					printf("Error: Gas accretion rate_0 value is not valid!\n");
					return 0;
				}
		...
		if(strcmp(sp, "Gas stellar luminosity_0 =") == 0){
			if(st == 0){
				er = fscanf (paramfile, "%lf", &P.G_L_s0);
				if(er <= 0 || P.G_L_s0 < 0){
					printf("Error: Gas stellar luminosity_0 value is not valid!\n");
					return 0;
				}
		...
		if(strcmp(sp, "Gas disk_viscous alpha =") == 0){
			if(st == 0){
				er = fscanf (paramfile, "%lf", &P.G_diskvis_alpha);
				if(er <= 0 || P.G_diskvis_alpha < 0){
					printf("Error: Gas disk_viscous alpha value is not valid!\n");
					return 0;
				}
	...
	//check output format
	{
		...
		for(int f = 0; f < def_Ninformat; ++f){
			...
			else if(GSF[st].outformat[f] == 9);//rho
			...
			else if(GSF[st].outformat[f] == 23); //a
			else if(GSF[st].outformat[f] == 24); //e
			else if(GSF[st].outformat[f] == 25); //inc
			else if(GSF[st].outformat[f] == 26); //O
			else if(GSF[st].outformat[f] == 27); //w
			else if(GSF[st].outformat[f] == 28); //M
...
//printf("restart %lld %d\n", P.tRestart, RestartBackup);
	...
	for(int st = 0; st < Nst; ++st){
		...
		if(P.tRestart > 0 && P.FormatP == 1){
			if(Nst == 1 || P.FormatS == 0){
				if(P.FormatT == 0){
					...
					sprintf(GSF[st].inputfilename, "%sOut%s_%.*lld.%s", GSF[st].path, GSF[st].X, def_NFileNameDigits, P.tRestart, dat_bin);//输入
		...
		if(P.tRestart == 0){
			printf("\n \n \n \n \n \n \n \n ");//这一块的sprintf似乎是用来写进缓存区的，foprn(w)或许才是储存
			printf("%s\n",GSF[st].logfilename);
			FILE *tfile;
...
__host__ int Host::readOutLine(double &time, int &index, double4 &x, double4 &v, double4 &spin, double3 &love, double3 &migration, float4 &aelimits, double &skip, double &aecount, unsigned long long &enccountT, double &rcrit, double &test, FILE *infile, int st){
	...
	for(int f = 0; f < def_Ninformat; ++f){
		...
		if(P.OutBinary == 0){
			...
			else if(ff == 23)	;//er = fscanf (infile, "%lf",&a);
			else if(ff == 24)	;//er = fscanf (infile, "%lf",&e);
			else if(ff == 25)	;//er = fscanf (infile, "%lf",&inc);
			else if(ff == 26)	;//er = fscanf (infile, "%lf",&O);
			else if(ff == 27)	;//er = fscanf (infile, "%lf",&w);
			else if(ff == 28)	;//er = fscanf (infile, "%lf",&M);
		}
		else{
...
//printf("Search for fragments %s\n", GSF[st].fragmentfilename);
			...
			char infilename[384];
			if(P.OutBinary == 0){
				sprintf(infilename, "%sOut%s_p%.6d.dat", GSF[st].path, GSF[st].X, i);//写入
				infile = fopen(infilename, "r");
			}
			else{
				sprintf(infilename, "%sOut%s_p%.6d.bin", GSF[st].path, GSF[st].X, i);//写入
