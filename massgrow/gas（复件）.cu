...
//r1 = id * 0.01;
		...
		if(r1 > rg0 && r1 < rg1 && x4.z/r1 < 1.5){ //otherwise there is no gas
			...
			//*********************************
			double mgo=30.0*pow((hgas/0.05),2.5)*pow((G_alpha/0.001),0.5)*(Msun/def_StarK2);//earth mass as before, I havent change it
			...
			double p,q;
			double mdot_gas=1e-8;
			double kap=1;
			p=-15.0/14.0;
			q=2.0/7.0;
			double Omega = ((Msun/pow(r1,3)),0.5) ;
			double hgas0_irr=(2.45e-2)*pow(L_s/1.0,1.0/7.0)*pow(M_s/1.0,-4.0/7.0);
			double hgas_irr = hgas0_irr*pow(r,q);
			double v_0=G_alpha*(hgas_irr,2)*Omega;
			double ts_0=1.0/v_0/(3.0*pow(p+2,2));
			//viscous heated region
			p=-0.375;
			q=-1.0/16.0;
			//double siggas0_vis=740;//mdot_gas是吸积，要查,kap也是,先默认随便取了
			double siggas0_vis=740*pow(mdot_gas/1.0e-8,1.0/2.0)*pow(M_s/1.0,1.0/8.0)*pow(G_alpha/1e-3,-3.0/4.0)*pow(kap/1e-2,-1.0/4.0);
			double siggas_vis=siggas0_vis*pow(r,p);
			//double hgas0_vis=4.5e-2;
			double hgas0_vis=4.5e-2*pow(mdot_gas/1e-8,1.0/4.0)*pow(M_s/1.0,-5.0/16.0)*pow(G_alpha/1e-3,-1.0/8.0)*pow(kap/1e-2,1.0/8.0);
			double hgas_vis=hgas0_vis*pow(r,q);
			//double T0_vis=500.0;
			double T0_vis=500.0*pow(mdot_gas/1e-8,1.0/2.0)*pow(M_s/1.0,3.0/8.0)*pow(G_alpha/1e-3,-1.0/4.0)*pow(kap/1e-2,1.0/4.0);
			double T_vis=T0_vis*pow(r,2*q-1);
			//stellar irradition region
			p = - 15.0/14.0;
			q = 2.0/7.0;
			//double siggas0_irr=2500;
			double siggas0_irr = 2500*(mdot_gas/1e-8)*(L_s/1.0,-2.0/7.0)*pow(M_s/1.0,9.0/14.0)*pow(G_alpha/1e-3,-1);
			...
			double rtrans = pow(500.0/150.0,56.0/39.0)*pow(mdot_gas/1e-8,28.0/39.0)*pow(M_s/1.0,29.0/39.0)*(G_alpha/1e-3,-14.0/39.0)*pow(L_s/1.0,-16.0/39.0)*pow(kap/1e-2,14.0/39.0);
			//double rtrans = 60; 
			double fs; 
			...
			double MSUN=1.989e33;//需要确认单位这里用的是g
			siggas = siggas/MSUN*pow(def_AU,2)*def_StarK2; //in solar mass*K2，这里原文MSUN*AU**2*K2，注意是单位换算
			siggas_vis = siggas_vis/MSUN*pow(def_AU,2)*def_StarK2;  //in solar mass*K2
			siggas_irr = siggas_irr/MSUN*pow(def_AU,2)*def_StarK2; //in solar mass*K2
...
//printf("%d %g %g %g %g %g | %g\n", id, r1, Sigma, h, a_r, a_z, erf(zh));
			...
			//TYPE I migration
			//*********************
			double tauwave = (Msun/m)*(Msun/(Sigma*pow(r1,2)))*pow(hgas,4)/Omega;//this sigma is gassigma, i dont know if it is rho
			//
			double f_tot=1.0e-20;//if no migration it is a small number
			f_tot = f1;//only type I
			f_tot  = f1*fs+f2*(1.0-fs)/pow(m_pl/mgo,2);
			//
			//migration timescale f_tot>0, outward migration; f_tot<0, inward migration 
			double taumig = 2.0*tauwave/f_tot/pow(hgas,2);
			double tauecc = tauwave/0.78/abs(f_tot);
			double tauinc = tauwave/0.544/abs(f_tot);
			...
			double Omega_c[50];
			double rextend_c[50];
			double rand1,rand2,rand3,rand4,rand5;//随机数但是不确定怎么放进来
			curandState state;
