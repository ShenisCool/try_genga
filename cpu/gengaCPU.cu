...
#if def_RV > 0
	memset(D.NRVT_h, 0, D.Nst * sizeof(int2));
#endif
#if def_TTV > 0
	memset(D.NtransitsT_h, 0, D.NconstT * sizeof(int2));
...
for(ittv = 0; ittv < D.P.TransitSteps; ++ittv){
	...
	memset(D.Nencpairs_h, 0, (D.Nst + 1) * sizeof(int));
	if(def_TTV == 1 || D.P.PrintTransits > 0){
		memset(D.TransitTime_h, 0, def_NtransitTimeMax * D.NconstT * sizeof(double));
	}
 #if def_TTV == 2
	setTimeTTV_kernel <<< (D.NconstT + 127) / 128  ,128 >>>(D.time_h, D.dt_h, D.lastTransitTime_d, D.transitIndex_d, D.EpochCount_d, D.TTV_d, D.ict_h[0] * 365.25, D.idt_h[0], D.Nst, D.NconstT);
 #endif
...
{
    #endif
	setJ_kernel <<< (Nst + 127) / 128, 128 >>>(D.random_h, D.elementsP_d, D.elementsI_d, D.elementsC_d, D.Nst, D.N_h[0], D.Msun_h, D.elementsM_d, ittv, D.P.mcmcNE, 4);
}
	SetTTVP1 <<< (D.NT + 127) / 128, 128 >>> (D.n1_h, D.rcrit_h, D.rcritv_h, D.index_h, D.n1_h[0], D.NT, D.N_h[0], D.Nst);
		
	if(ittv <= 0) D.modifyElementsCall(ittv, 0); //initialize ensemble walkers
	else D.modifyElementsCall(ittv, 4);
	HelioToDemo_kernel <<< (D.Nst + 127) / 128, 128 >>> (D.x4_h, D.v4_h, D.NBS_h, D.Msun_h[0].x, D.Nst, D.N_h[0]);
	//HelioToBary_kernel <<< (D.Nst + 127) / 128, 128 >>> (D.x4_h, D.v4_h, D.NBS_h, D.Msun_h[0].x, D.Nst, D.N_h[0]);
...
//printf("-----\n");
	...
	if(ittv == 0){
		...
		rmsPropRand <<< (D.Nst / (D.N_h[0] * D.P.mcmcNE + 1) + 127) / 128  ,128 >>> (D.random_h, D.elementsAOld_d, D.elementsBOld_d, D.elementsTOld_d, D.elementsL_d, D.N_h[0], D.P.mcmcNE, D.Nst);
	}
	...
	//HelioToBary_kernel <<< (D.Nst + 127) / 128, 128 >>> (D.x4_h, D.v4_h, D.NBS_h, D.Msun_h[0].x, D.Nst, D.N_h[0]);
  #endif
	memcpy(D.elementsA_h, D.elementsA_d, sizeof(double4) * D.NconstT);
#endif
...
#if def_RV > 0
	memcpy(D.NRVT_h, D.NRVT_d, D.Nst * sizeof(int2));
...
#endif
#if def_TTV > 0
	memcpy(D.NtransitsT_h, D.NtransitsT_d, D.NconstT * sizeof(int2));
 ...
 #if def_TTV == 1
	TTVstep <<< (D.NT + 255) / 256, 256 >>> (D.TransitTime_d, D.TransitTimeObs_d, D.NtransitsT_d, D.NtransitsTObs_d, D.N_h, D.elementsT_d, D.NT, ittv);
 #endif
 ...
 #if def_TTV == 1
	TTVstep1 < HCM_Bl, HCM_Bl2, NmaxM > <<< (D.NT + HCM_Bl2 - 1) / HCM_Bl2, HCM_Bl >>> (D.index_h, D.TransitTime_d, D.RVP_d, D.elementsP_d, D.NtransitsT_d, D.n1_h, D.NT, D.N_h[0], D.Nst);
 #endif
 #if def_TTV == 2
	TTVstepb <<< (D.Nst + 127) / 128, 128 >>> (D.elementsP_d, D.TTV_d, D.NtransitsT_d, D.EpochCount_d, D.Nst, D.N_h[0]);
 #endif

	TTVstep3 <<< (D.NT + 255) / 256, 256 >>> (D.index_h, D.elementsA_d, D.elementsB_d, D.elementsT_d, D.elementsSpin_d, D.elementsAOld_d, D.elementsBOld_d, D.elementsTOld_d, D.elementsSpinOld_d, D.elementsP_d, D.elementsSA_d, D.elementsC_d, D.NtransitsT_d, D.Msun_h, D.elementsM_d, D.NT, D.N_h[0], D.Nst, D.P.mcmcNE);
 ...
 #if MCMC_Q == 2
 if(ittv > 0) TTVstepRefine <<< (D.NT + 255) / 256, 256 >>> (D.TransitTime_d, D.TransitTimeObs_d, D.NtransitsT_d, D.NtransitsTObs_d, D.N_h, D.elementsT_d, D.NT, ittv);
 #endif
