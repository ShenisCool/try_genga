...
//
// Use vold_h as temporary storage for Spinsun
//
// October 2021
// Authors: Simon Grimm, Jean-Baptiste Delisle
// **********************************************************
void force_cpu(double4 *x4_h, double4 *v4_h, int *index_h, double4 *spin_h, double3 *love_h, double2 *Msun_h, double4 *Spinsun_h, double3 *Lovesun_h, double2 *J2_h, double4 *vold_h, double *dt_h, const double Kt, double *time_h, const int N, const int Nst, const int UseGR, const int UseTides, const int UseRotationalDeformation, const int Nstart, const int si){
	...
	for(id = 0 * 1 + 0 + Nstart; id < N + Nstart; ++id){
		...
		double4 x4 = x4_h[id];
		double4 v4 = v4_h[id];
		double4 Spin = spin_h[id];
		double Msun = Msun_h[st].x;			//This is the mass of the central star
		double4 Spinsun = Spinsun_h[st];		//This is the spin of the central star and the moment of inertia
		double3 Lovesun = Lovesun_h[st];		//This is the Love number, fluid Love numer and time lag
		double2 J2s = J2_h[st];			//This is the J2 value for additional gravitational harmonics forces and the mean radius
		dt = dt_h[st] * Kt;			//This is the time step to do
//		double time = time_h[st] / 365.25;		//This is the time in years
		...
		if((UseTides == 2) && x4.w > 0.0){
			...
			double E = -3.0 * love_h[id].x * def_ksq * Msun * Msun * R5 / x4.w * ir7;
//printf("%d %g %g\n", id, E * ir, E * x4.x * ir);
			a3.x += E * x4.x * ir;
			a3.y += E * x4.y * ir;	
			a3.z += E * x4.z * ir;
		...
		if(UseRotationalDeformation == 2){
			...
			double Rsun2 = Msun_h[st].y * Msun_h[st].y;
			double Rsun5 = Rsun2 * Rsun2 * Msun_h[st].y;
		...
		if((UseTides == 1 || UseRotationalDeformation == 1 || J2s.x != 0.0) && x4.w >= 0.0){
			Rsun2 = Msun_h[st].y * Msun_h[st].y;
			Rsun3 = Rsun2 * Msun_h[st].y;
			Rsun5 = Rsun3 * Rsun2;
...
//printf("omegaP %d %g %g %g\n", id, omega3.x * dayUnit, omega3.y * dayUnit, omega3.z * dayUnit);
		...
		if((UseTides == 1) && x4.w > 0.0){
			...
			double m2 = x4.w * x4.w;
			double love = love_h[id].x;
			double tau = love_h[id].z;
...
//printf("B1 %d %g %g %g\n", id, T3sun.x * dt, T3sun.y * dt, T3sun.z * dt);
			...
			if(N <= 1 && id == 0){
				...
				if(si == 1){
					Spinsun_h[st].x += T3sun.x * dt;
					Spinsun_h[st].y += T3sun.y * dt;
					Spinsun_h[st].z += T3sun.z * dt;
				}
			}
			else if(0 == 0){
				vold_h[0].x = T3sun.x * dt;
				vold_h[0].y = T3sun.y * dt;
				vold_h[0].z = T3sun.z * dt;
			}

		}
		else for(id = 0 * 1 + 0 + Nstart; id < N + Nstart; ++id){ //Nst > 1
			vold_h[id].x = T3sun.x * dt;
			vold_h[id].y = T3sun.y * dt;
			vold_h[id].z = T3sun.z * dt;
		}
#else
		if(si == 1){
			Spinsun_h[st].x += T3sun.x * dt;
...
int setElementsNumbers_c[3];
int setElements_c[25];
//**************************************
...
//Authors: Simon Grimm
//***************************************/
void Host::constantCopy3(int *Elements, const int nelements, const int nbodies, const int nlines){
...
// modifies the Keplerian elements according to the setElementsData_h data and
...
// Authors: Simon Grimm
// *****************************************************************
void setElements_cpu(double4 *x4_h, double4 *v4_h, int *index_h, double *setElementsData_h, int *setElementsLine_h, double2 *Msun_h, double *dt_h, double *time_h, const int N, const int Nst, const int EE){

	int id = 0 * 1 + 0;

	int line = setElementsLine_h[0];
	int nelements = setElementsNumbers_c[0];
	int nbodies = setElementsNumbers_c[1];
	int nlines = setElementsNumbers_c[2];


	for(id = 0 * 1 + 0; id < nbodies; ++id){
...
//printf("id %d, line %d, nelements %d, nbodies %d EE %d\n", id, line, nelements, nbodies, EE);
		...
		if(Nst > 1 && id < N) st = index_h[id] / def_MaxIndex;	//st is the sub simulation index
...
//printf("elements %d %d\n", i, setElements_c[i]);
		...
		if(x4i.w >= 0.0){
			...
			double time;
			if(EE == 0){
				time = (time_h[st] - dt / dayUnit) / 365.25;//time at beginning of the time step in years
			}
			else{
				time = time_h[st] / 365.25;//time at end of the time step in years
			}
...
//printf("K0 %d %.10g %.10g %g %g %g %g %g %g\n", id, x4i.w, a, e, inc, Omega, w, E, Theta);
			...
			for(int i = 0; i < nelements; ++i){
				if(setElements_c[i] == 1){
					time0 = setElementsData_h[line * nelements + i];
					time1 = setElementsData_h[line1 * nelements + i];
					time2 = setElementsData_h[line2 * nelements + i];
					time3 = setElementsData_h[line3 * nelements + i];
...
//printf("setLine %d %d\n", id, line);			
			...
			for(int i = 0; i < nelements; ++i){
				...
				xx0 = setElementsData_h[(line + id) * nelements + i];
				xx1 = setElementsData_h[(line1 + id) * nelements + i];
				xx2 = setElementsData_h[(line2 + id) * nelements + i];
				xx3 = setElementsData_h[(line3 + id) * nelements + i];
...
// rotation reset. 
// Fragmentation events are reportend in the Fragments_h array.
//
// March 2017
// Authors: Simon Grimm, Matthias Meier
// *****************************************************************
void rotation_cpu(int *random_h, double4 *x4_h, double4 *v4_h, double4 *spin_h, int *index_h, int *N_h, int *Nsmall_h, double *dt_h, const int st, double *Fragments_h, double time, int *nFragments_m){
	...
	for(id = 0 * 1 + 0 + N; id < Nsmall + N; ++id){

		volatile double4 x4 = x4_h[id];
		volatile double4 v4 = v4_h[id];
		double4 spin = spin_h[id];
		...
		if(x4.w >= 0.0){
			...
			if(rd < p && omega > 0.0) {
#if def_CPU == 0
				accept = atomicMax(&nFragments_m[0], 0);
#else
				accept = nFragments_m[0];
#endif

printf("rotation reset %d %d %g %g %g\n", id, index_h[id], time/365.25, rd, p);
...
printf("rB %g %d %g %g %g %g\n", time, id, RR, omega, p, rd);
				...
				Fragments_h[0] = time/365.25;
				Fragments_h[1] = (double)(index_h[id]);
				Fragments_h[2] = x4_h[id].w;
				Fragments_h[3] = v4_h[id].w;
				Fragments_h[4] = x4_h[id].x;
				Fragments_h[5] = x4_h[id].y;
				Fragments_h[6] = x4_h[id].z;
				Fragments_h[7] = v4_h[id].x;
				Fragments_h[8] = v4_h[id].y;
				Fragments_h[9] = v4_h[id].z;
				Fragments_h[10] = spin_h[id].x;
				Fragments_h[11] = spin_h[id].y;
				Fragments_h[12] = spin_h[id].z;

#if def_CPU == 0
				atomicMax(&nFragments_m[0], 1);
#else
				nFragments_m[0] = 1;
#endif
			}
...
// Authors: Simon Grimm, Matthias Meier
// *****************************************************************
void fragment_cpu(int *random_h, double4 *x4_h, double4 *v4_h, double4 *spin_h, double3 *love_h, int *index_h, int *N_h, int *Nsmall_h, double *dt_h, const int NconstT, const int MaxIndex, const int st, double *Fragments_h, double time, int *nFragments_m){
#if USE_RANDOM == 1
	int N = N_h[st];
	...
	for(id = 0 * 1 + 0 + N; id < Nsmall + N; ++id){ 
		...
		if(x4.w >= 0.0){
			...
			if(rd < p) {
				...
				accept = nFragments_m[0];
#endif
printf("fragment %d %d %d %g %g %g %g %g %d\n", id, index_h[id], accept, time/365.25, rd, p, M, RR, MaxIndex);
			}
			if(accept == -1){
				...
				for(ii = 0; ii < 10000; ++ii){
					...
					double vscale = drand48() * 2.0 * 0.2 + (1.0 - 0.2) * pow(m, -1.0/6.0);
					vscaleT += vscale;
					//velocity
...
printf("fB %d %g %g %g %g %g %g %g %g %g\n", ii, M, RR, m, r, v, vx, vy, vz, v4.x);
					...
					x4_h[ii + N + Nsmall].x = x4.x + x;
					x4_h[ii + N + Nsmall].y = x4.y + y;
					x4_h[ii + N + Nsmall].z = x4.z + z;
					x4_h[ii + N + Nsmall].w = m;

					v4_h[ii + N + Nsmall].x = vx;
					v4_h[ii + N + Nsmall].y = vy;
					v4_h[ii + N + Nsmall].z = vz;
					v4_h[ii + N + Nsmall].w = r;
					...
					spin_h[ii + N + Nsmall] = spin;
					love_h[ii + N + Nsmall] = love;
					index_h[ii + N + Nsmall] = MaxIndex + ii + 1;


//printf("%.20g %d %.20g %.20g %.20g %.20g %.20g %.20g %.20g %.20g %.20g %.20g %.20g 0\n", time/365.25, index_h[ii + N + Nsmall], x4_h[ii + N + Nsmall].w, v4_h[ii + N + Nsmall].w, x4_h[ii + N + Nsmall].x, x4_h[ii + N + Nsmall].y, x4_h[ii + N + Nsmall].z, v4_h[ii + N + Nsmall].x, v4_h[ii + N + Nsmall].y, v4_h[ii + N + Nsmall].z, spin_h[ii + N + Nsmall].x, spin_h[ii + N + Nsmall].y, spin_h[ii + N + Nsmall].z);

					if(M == 0.0) break;
					if(N + Nsmall + ii >= NconstT){
						Nsmall_h[st] += ii;
#if def_CPU == 0
						atomicMax(&nFragments_m[0], ii);
#else
						nFragments_m[0] = ii;
#endif
						break;
				...
				for(int i = 0; i < ii; ++i){
					...
					double vx = v4_h[i + N + Nsmall].x / vscaleT;
					double vy = v4_h[i + N + Nsmall].y / vscaleT;
					double vz = v4_h[i + N + Nsmall].z / vscaleT;

					v4_h[i + N + Nsmall].x = v4.x + vx;
					v4_h[i + N + Nsmall].y = v4.y + vy;
					v4_h[i + N + Nsmall].z = v4.z + vz;


					Fragments_h[(i + 1) * 25 + 0] = time/365.25;
					Fragments_h[(i + 1) * 25 + 1] = (double)(index_h[i + N + Nsmall]);
					Fragments_h[(i + 1) * 25 + 2] = x4_h[i + N + Nsmall].w;
					Fragments_h[(i + 1) * 25 + 3] = v4_h[i + N + Nsmall].w;
					Fragments_h[(i + 1) * 25 + 4] = x4_h[i + N + Nsmall].x;
					Fragments_h[(i + 1) * 25 + 5] = x4_h[i + N + Nsmall].y;
					Fragments_h[(i + 1) * 25 + 6] = x4_h[i + N + Nsmall].z;
					Fragments_h[(i + 1) * 25 + 7] = v4_h[i + N + Nsmall].x;
					Fragments_h[(i + 1) * 25 + 8] = v4_h[i + N + Nsmall].y;
					Fragments_h[(i + 1) * 25 + 9] = v4_h[i + N + Nsmall].z;
					Fragments_h[(i + 1) * 25 + 10] = spin_h[i + N + Nsmall].x;
					Fragments_h[(i + 1) * 25 + 11] = spin_h[i + N + Nsmall].y;
					Fragments_h[(i + 1) * 25 + 12] = spin_h[i + N + Nsmall].z;

//printf("%.20g %d %.20g %.20g %.20g %.20g %.20g %.20g %.20g %.20g %.20g %.20g %.20g 0\n", time/365.25, index_h[i + N + Nsmall], x4_h[i + N + Nsmall].w, v4_h[i + N + Nsmall].w, x4_h[i + N + Nsmall].x, x4_h[i + N + Nsmall].y, x4_h[i + N + Nsmall].z, v4_h[i + N + Nsmall].x, v4_h[i + N + Nsmall].y, v4_h[i + N + Nsmall].z, spin_h[i + N + Nsmall].x, spin_h[i + N + Nsmall].y, spin_h[i + N + Nsmall].z);

					//remove too small particles
					double r = v4_h[i + N + Nsmall].w;
					if(r * def_AU < Asteroid_rdel_c[0]){
printf("Remove Fragment %d %g\n", i + N + Nsmall, r * def_AU);
						x4_h[i + N + Nsmall].x = 1.0;
						x4_h[i + N + Nsmall].y = 0.0;
						x4_h[i + N + Nsmall].z = 0.0;
						x4_h[i + N + Nsmall].w = -1.0e-12;

						v4_h[i + N + Nsmall].x = 0.0;
						v4_h[i + N + Nsmall].y = 0.0;
						v4_h[i + N + Nsmall].z = 0.0;
						v4_h[i + N + Nsmall].w = 0.0;

						spin_h[i + N + Nsmall].x = 0.0;
						spin_h[i + N + Nsmall].y = 0.0;
						spin_h[i + N + Nsmall].z = 0.0;
						spin_h[i + N + Nsmall].w = 0.4;
	
						love_h[i + N + Nsmall].x = 0.0;
						love_h[i + N + Nsmall].y = 0.0;
						love_h[i + N + Nsmall].z = 0.0;
					}
				}
...
void Data::fragmentCall(){
	if(Nsmall_h[0] > 0.0){
		int st = 0;
		nFragments_m[0] = -1;
		fragment_cpu /* (Nsmall_h[0] + 255) / 256, 256 */ (random_h, x4_h, v4_h, spin_h, love_h, index_h, N_h, Nsmall_h, dt_h, NconstT, MaxIndex, st, Fragments_h, time_h[0], nFragments_m);
		if(nFragments_m[0] > 0){
#if def_CPU == 0
...
void Data::rotationCall(){
	if(Nsmall_h[0] > 0.0){
		int st = 0;
		nFragments_m[0] = -1;
		rotation_cpu /* (Nsmall_h[0] + 255) / 256, 256 */ (random_h, x4_h, v4_h, spin_h, index_h, N_h, Nsmall_h, dt_h, st, Fragments_h, time_h[0], nFragments_m);
	}
}
...
// Authors: Simon Grimm, Matthias Meier
// *****************************************************************
void CallYarkovsky2_cpu(double4 *x4_h, double4 *v4_h, double4 *spin_h, int *index_h, double2 *Msun_h, double *dt_h, const double Kt, const int N, const int Nst, const int Nstart){
...
//if(id == 0) printf("Asteroid %g %g %g %g %g %g\n", Asteroid_eps_c[0], Asteroid_rho_c[0], Asteroid_C_c[0], Asteroid_A_c[0], Asteroid_K_c[0], Asteroid_V_c[0]);
	for(id = 0 * 1 + 0 + Nstart; id < N + Nstart; ++id){
		...
		if(Nst > 1) st = index_h[id] / def_MaxIndex;	//st is the sub simulation index
...
// Authors: Simon Grimm, Matthias Meier
// *****************************************************************
void PoyntingRobertsonEffect_averaged_cpu(double4 *x4_h, double4 *v4_h, int *index_h, double2 *Msun_h, double *dt_h, const double Kt, const int N, const int Nst, const int Nstart){
...
// Authors: Simon Grimm, Matthias Meier
// *****************************************************************
void PoyntingRobertsonEffect2_cpu(double4 *x4_h, double4 *v4_h, int *index_h, double *dt_h, const double Kt, const int N, const int Nst, const int Nstart){
...
void artificialMigration_cpu(double4 *x4_h, double4 *v4_h, int *index_h, double3 *migration_h, double2 *Msun_h, double *dt_h, double Kt, const int N, const int Nst, const int Nstart, const int si){
	...
	for(id = 0 * 1 + 0 + Nstart; id < N + Nstart; ++id){
		...
		double4 x4 = x4_h[id];
		double4 v4 = v4_h[id];
		double3 migration = migration_h[id];		//migration time scales in 1/year
...
void artificialMigration2_cpu(double4 *x4_h, double4 *v4_h, int *index_h, double *dt_h, const double Kt, const int N, const int Nst, const int Nstart, const int si){
